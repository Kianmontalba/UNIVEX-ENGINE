...
TEST(EditorBridgeUVETest, SnapshotUVE_CopiesHierarchyInspectorAndNativePanelSessionState) {
    ...
    ASSERT_TRUE(engine.Load());
    {
        ...
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[12], "math.vector3.make");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[19], "math.vector3.normalize");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[23], "logic.boolean.xor");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[24], "logic.boolean.equal");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[25], "logic.boolean.not_equal");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[26], "logic.boolean.greater");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[27], "logic.boolean.less");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[28], "logic.boolean.greater_equal");
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[29], "logic.boolean.less_equal");
