...
TEST(EditorBridgeUVETest, SnapshotUVE_CopiesHierarchyInspectorAndNativePanelSessionState) {
    ...
    ASSERT_TRUE(engine.Load());
    {
        ...
        ASSERT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds.size(), 22U);
        ASSERT_EQ(snapshot.visualScripting.canvas.paletteDescriptors.size(), 22U);
        ...
        EXPECT_EQ(snapshot.visualScripting.canvas.paletteNodeTypeIds[20], "engine.log");
