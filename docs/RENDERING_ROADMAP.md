...
| **COMPLETED** | **Increments 22–26** | Textured materials, directional/point/spot lights, PBR uniform plumbing, and shadow mapping. | This is a renderer foundation, not a complete asset-material workflow; LightComponent validity and runtime extraction preconditions are now explicit, while ranking, attenuation, and advanced lighting remain partial. |
...
| **COMPLETED** | **Legacy Partial Slice — Renderer Asset-Resolution Diagnostics v1** | Mesh extraction reports invalid references, pending loads, and failed loads in the copied RenderQueue/Renderer3D frame snapshot; failed texture resolution reports renderer-owned fallback use. | Counts explain safe skip/fallback decisions without exposing GPU handles or mutating AssetManager/ECS state; editor-facing preview presentation remains PARTIAL. |
| **COMPLETED** | **Legacy Partial Slice — Raw Asset Source Classification v1** | Importer policy classifies raw model, texture, and material source extensions and publishes explicit parser-required/unsupported diagnostics without pretending to convert source data. | Renderer integration remains downstream: actual parser selection, licensing, source-to-derived-data transforms, and preview generation require separate bounded increments. |
...
| **PARTIAL** | **3** | Material and mesh asset workflow | Asset-backed mesh/material GUID references appear as copied read-only Inspector facts, existing typed UVE envelopes have deterministic copy-register import contracts, raw model/texture/material source classification exposes parser-required diagnostics, AssetManager failures expose copied diagnostics, and Renderer3D reports per-frame invalid/pending/failed/fallback counts. The remaining workflow still requires actual raw conversion backends, registered Content Browser import/reimport actions, and renderer-backed asset previews. |
