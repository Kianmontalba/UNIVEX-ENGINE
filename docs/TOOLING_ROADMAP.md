...
| **PARTIAL** | Visual scripting core | Native C++ graph model, node registry, typed pins/links, compiler, bytecode, VM, bindings, debugger, and script state. | Stable scene/component/property contracts; no C# runtime or managed bridge is implied. |
...
| **PARTIAL** | Plugin architecture | Native plugin manifests, lifecycle, versioning, registration scopes, and safe load/unload behavior. | A separately designed ABI/lifecycle/security proposal; plugins do not become a shortcut around engine ownership. |
