...
| **IMPLEMENTING** | **Increment 93 — Read-Only Data Table Catalog Bridge v1** | Bounded native catalog DTOs, additive stdio serialization, revision synchronization, backward-compatible managed parsing, and a presentation-only Avalonia Data Tables tab. | No catalog mutation request, filesystem access, asset-database ownership, editor-side table editing, runtime binding, references, hot reload, or visual-scripting node is introduced. |
