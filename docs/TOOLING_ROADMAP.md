...
| **COMPLETED** | **Increment 228 — Motion Query Live Debug Trace Event Categorization v1** | Implement event categorization, category filter logic, bridge serialization, and managed UI categorization controls. | `category` field in trace event; `SetCategoryUVE` in trace logger; `SetTraceEventCategory` live-debug command; expanded filter logic; managed UI category TextBox; all native and managed tests passed. |
| **COMPLETED** | **Increment 229 — Motion Query Live Debug Trace Persistence v1** | Export and import bounded annotated live-debug traces through a strict value-only JSON envelope and managed clipboard workflow. | Persistence omits database handles and timestamps, preserves comments/categories/pin state/filter/truncation, validates schema/fields/order/capacity atomically, and restores an offline inspection session. Full native CTest passed 1013/1013 and managed .NET 8 passed 71/71; seven expected shader parity cases remain skipped. |
| **COMPLETED** | **Legacy Partial Reconciliation — Rendering Increment 63** | Close the stale PARTIAL status for the first legacy viewport/render milestone while preserving the renderer-owned presentation boundary. | Added the bounded blue-gray gradient to the physical and embedded editor viewport shader, asserted the renderer editor-visual pass, passed full native/managed validation, and completed a real 30-frame OpenGL/Xvfb editor smoke run. |
| **COMPLETED** | **Legacy Partial Reconciliation — Selection and Viewport Visual Feedback** | Close the next renderer partial by proving bounded selection, gizmo-axis, and orientation facts flow through the existing editor visual GPU pass. | Renderer tests assert `uSelectionVisible`, `uActiveGizmoAxis`, and `uCameraForward` publication through the recorded RHI command stream; no new renderer owner or managed mutation path is introduced. |
| **COMPLETED** | **Legacy Partial Slice — Material/Mesh Inspector Asset Binding v1** | Publish selected MeshComponentUVE mesh/material AssetGuid values through the native Inspector, stdio bridge, managed DTO, and Avalonia presentation. | Native bridge and stdio tests verify deterministic values; managed tests verify the additive field and legacy payload compatibility. Asset loading, import, ECS mutation, and ownership remain native and are still tracked by the Rendering material/mesh PARTIAL. |
