...
| **IMPLEMENTING** | **Increment 97 — Read-Only Data Table Preview Selection Protocol v1** | Named revision-checked native selection request, stdio JSON parsing, managed command serialization, and Avalonia catalog selection for copied native preview facts. | Selection changes presentation state only. Unknown names, stale revisions, registry-free bridge sessions, and malformed payloads reject without table mutation or pointer transfer. |
