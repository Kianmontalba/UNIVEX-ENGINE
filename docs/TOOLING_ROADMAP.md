...
| **IMPLEMENTING** | **Increment 99 — Validated Data Table Asset-Database Registration v1** | Explicit helper validates one `.uvetable` envelope before registering its lexical path and returning an existing or generated GUID. | Invalid extensions, missing files, wrong envelope kinds, and malformed payloads reject before database mutation. No directory scan, import queue, hot reload, runtime binding, or managed pointer. |
