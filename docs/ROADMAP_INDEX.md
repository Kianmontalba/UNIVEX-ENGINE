...
|---|---:|---|
| Core Runtime broad PARTIAL families | **1** | The authoritative engine queue after Platform abstraction Increment 661: networking. |
| Native C++ Visual Scripting | **COMPLETED** | Closed by Increment 650; it is not included in the one remaining Core Runtime family. |
...
## Core Runtime — one remaining broad PARTIAL family
...
The one family below is the authoritative Core Runtime queue after Platform abstraction Increment 661. The owner-facing metric is **1 engine-only broad PARTIAL family**; Motion Query and Control Rig remain separate completed plugin families.
