...
|---|---|---|---|
| **COMPLETED / PARTIAL** | [Complete Development Roadmap](EDITOR_ROADMAP.md) | Every completed Increment **1–71**, immediate Scene Editor increments, and the first-generation editor completion gate. | Use the completed managed fixed-region shell and versioned bridge transport to scope C# Hierarchy, Inspector, and Content Browser presentation without bypassing native command, scene, or renderer ownership. |
...
| **COMPLETED / PARTIAL** | [Editor & Workflow Roadmap](EDITOR_ROADMAP.md) | Scene authoring, selection, gizmos, Inspector, FileSystem, import monitoring, preferences, versioned bridge transport, managed host foundation, and fixed C# shell layout. | Extend C# panels only through copied bridge DTOs and native command routing; preserve explicit-save-only managed layout and keep stdio bridge mode headless-only until an explicit co-hosting contract exists. |
