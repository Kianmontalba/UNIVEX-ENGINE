...
|---|---:|---|
| Core Runtime broad PARTIAL families | **6** | The authoritative engine queue after Scene Components Increment 654, Prefab Maturity Increment 655, and Asset-type Importers Increment 656: physics, audio, input, save, platform, and networking. |
| Native C++ Visual Scripting | **COMPLETED** | Closed by Increment 650; it is not included in the six remaining Core Runtime families. |
...
The six families above are the authoritative Core Runtime queue after Increment 656. The owner-facing metric is **6 engine-only broad PARTIAL families**; Motion Query and Control Rig remain separate completed plugin families.
