...
|---|---|---|---|
| **COMPLETED / PARTIAL** | [Complete Development Roadmap](EDITOR_ROADMAP.md) | Every completed Increment **1–69**, immediate Scene Editor increments, and the first-generation editor completion gate. | Use the completed versioned bridge contract to scope the C# Editor Host Foundation v1 without bypassing native editor ownership. |
...
| **COMPLETED / PARTIAL** | [Editor & Workflow Roadmap](EDITOR_ROADMAP.md) | Scene authoring, selection, gizmos, Inspector, FileSystem, import monitoring, preferences, and the versioned editor bridge. | Build a separate C# host only through copied bridge DTOs and native command routing. |
