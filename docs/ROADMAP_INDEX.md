...
|---|---|---|---|
| **COMPLETED / PARTIAL** | [Complete Development Roadmap](EDITOR_ROADMAP.md) | Every completed Increment **1–70**, immediate Scene Editor increments, and the first-generation editor completion gate. | Use the completed headless C# host and versioned bridge transport to scope the C# Editor Shell & Docking v1 without bypassing native command, scene, or renderer ownership. |
...
| **COMPLETED / PARTIAL** | [Editor & Workflow Roadmap](EDITOR_ROADMAP.md) | Scene authoring, selection, gizmos, Inspector, FileSystem, import monitoring, preferences, versioned bridge transport, and the managed host foundation. | Extend the C# shell only through copied bridge DTOs and native command routing; keep stdio bridge mode headless-only until an explicit co-hosting contract exists. |
