...
|---|---:|---|
| Core Runtime broad PARTIAL families | **2** | The authoritative engine queue after Save-game depth Increment 660: platform and networking. |
| Native C++ Visual Scripting | **COMPLETED** | Closed by Increment 650; it is not included in the two remaining Core Runtime families. |
...
## Core Runtime — two remaining broad PARTIAL families
...
The two families below are the authoritative Core Runtime queue after Save-game depth Increment 660. The owner-facing metric is **2 engine-only broad PARTIAL families**; Motion Query and Control Rig remain separate completed plugin families.
