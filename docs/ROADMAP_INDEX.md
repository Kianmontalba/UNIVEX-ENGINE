...
|---|---:|---|
| Core Runtime broad PARTIAL families | **3** | The authoritative engine queue after Input breadth Increment 659: save, platform, and networking. |
| Native C++ Visual Scripting | **COMPLETED** | Closed by Increment 650; it is not included in the three remaining Core Runtime families. |
...
## Core Runtime — three remaining broad PARTIAL families
...
The three families below are the authoritative Core Runtime queue after Input breadth Increment 659. The owner-facing metric is **3 engine-only broad PARTIAL families**; Motion Query and Control Rig remain separate completed plugin families.
