...
|---|---:|---|
| Core Runtime broad PARTIAL families | **4** | The authoritative engine queue after Audio depth Increment 658: input, save, platform, and networking. |
| Native C++ Visual Scripting | **COMPLETED** | Closed by Increment 650; it is not included in the four remaining Core Runtime families. |
...
The four families above are the authoritative Core Runtime queue after Audio depth Increment 658. The owner-facing metric is **4 engine-only broad PARTIAL families**; Motion Query and Control Rig remain separate completed plugin families.
