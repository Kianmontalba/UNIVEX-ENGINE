...
|---|---|---|---|
| **COMPLETED / PARTIAL** | [Complete Development Roadmap](EDITOR_ROADMAP.md) | Every completed Increment **1–74**, immediate Scene Editor increments, and the first-generation editor completion gate. | Hold new editor/Visual Scripting family expansion while the 9 Core Runtime partial areas remain; retain native C++ renderer/OpenGL ownership and the managed host's explicitly deferred viewport surface boundary. Increment 74 surface lifecycle is completed as an explicit typed unavailable/native-owned contract. |
| **PARTIAL** | [Core & Runtime Roadmap](CORE_RUNTIME_ROADMAP.md) | Engine loop, memory, threading, ECS, scene, asset, physics, input, audio, save, and platform foundations. | Finish the 9 authoritative partial areas—scene components, prefab maturity, asset-type importers, physics, audio, input/platform policy, save depth, platform abstraction, and networking—before starting new AAA families. |
| **PARTIAL** | [AAA Core Roadmap](AAA_CORE_ROADMAP.md) | Production/AAA-oriented future capabilities spanning animation, simulation, rendering, networking, audio, packaging, and hardening. | Remains a future planning authority; do not displace the legacy partial-first critical path, and do not promote planned capabilities without implementation and verification evidence. |
...
| **COMPLETED** | [Developer Tooling Roadmap](TOOLING_ROADMAP.md) | Visual scripting, plugins, project diagnostics, data tooling, console, CI, and developer documentation. | The documented tooling sequence has no remaining top-level PARTIAL milestones; future additions remain subject to the evidence-before-claims rule and the legacy partial-first critical path. |
...
| **ACTIVE** | [Loops Execution Queue](LOOPS_EXECUTION_QUEUE.md) | Autonomous dependency-ordered execution queue across legacy partials, AAA future systems, and the DOCX Motion Query inventory. | One bounded increment per loop, red/orange/green review gates, full validation, signed commits, CI, and merge before continuing. |
