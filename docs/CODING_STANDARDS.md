...
| `UVE::Asset`      | `engine/asset/`      | `AssetGuidUVE`, `AssetDatabaseUVE`, `ProjectFileIndexUVE`, `AssetManagerUVE`, `AssetImporterUVE`, `HotReloadUVE`, `AssetBundleUVE`, `FileSystemUVE`, the `.uve*` binary envelope |
...
The fixed workspace layout has a compact **UNIVEX** header with session-only Library, Asset, Scripting, Debug, and Plugin labels; a left **Scene** panel; an interactive transparent **3D Viewport** in the center; a right panel with horizontally arranged **Inspector**, **Import**, and **Signals** tabs; and a lower dock. The active lower dock defaults to **FileSystem**, which presents the cached read-only `ProjectFileIndexUVE` snapshot for the configured project content root. Debugger, Animator, and AI Toolbar are selectable dock labels only until separate runtime diagnostics, animation, and AI-tooling contracts are implemented. None of these workspace or panel-selection values are serialized, added to Undo/Redo, or allowed to mutate document state by themselves. Hierarchy labels prefer a persistent `NameComponentUVE` when present and otherwise retain the stable entity-index/generation fallback for legacy and runtime-created entities. Selection has one shared owner in `EditorUVE`: hierarchy clicks, viewport picking, the Inspector tab, and the translate gizmo all operate on the same live `m_selectedEntity`. The Inspector tab supports exactly one live selected entity and writes its Name metadata plus existing local Transform through validated editor paths, preserving dirty propagation into derived world transforms. Invalid, deleted, non-transform, non-finite, or invalid-name edits are rejected without mutating ECS state.
...
**The FileSystem dock is read-only session UI.** It renders a filterable folder tree, retains visible matching ancestors, exposes a session-only selected entry, and reports directory/file kind plus optional registered GUID. It stays usable in Play/Pause because filtering, Refresh, and selection do not mutate document state, dirty state, Undo/Redo, or sandbox state. The dock deliberately does not implement import/reimport, previews/thumbnails, drag-and-drop, rename/move/delete/create-folder operations, native file dialogs, directory watching, or source-control commands. Those features require separate ownership and transactional policies in later increments.
...
The bottom **FileSystem** dock now exposes the cached ProjectFileIndex tree through a case-insensitive path filter, session-only selected project entry, and optional GUID detail. Its editor state retains only copied entry/record values and filter text, so it does not retain AssetDatabase internals or alter asset ownership. Scene persistence remains available through the existing editor command API; the workspace update does not introduce a separate content serialization format or a native file-dialog dependency.
