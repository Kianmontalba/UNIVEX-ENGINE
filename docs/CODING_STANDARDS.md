...
**All bridge mutations delegate to existing public `EditorUVE` command paths.** Selection, clear selection, selected-name editing, document-entity creation, Undo, and Redo preserve the native command layer's existing lifecycle, document-entity, Play Mode, dirty-state, validation, and history behavior. A future C# host must never write `.uvescene` files, mutate ECS components directly, own a GL context/resource, or bypass these commands. Native ImGui and a bridge client may coexist: a client must fetch a fresh snapshot after any revision mismatch rather than assuming it is the only control surface.
