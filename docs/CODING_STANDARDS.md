...
**The managed host owns only its child process and copied response values.** It launches `uve_editor --bridge-stdio` through an argument list, drains stderr separately, performs `bridge.hello` before reporting a connected state, and closes streams before a bounded child-only termination fallback. There is no automatic reconnect or restart. A backend crash, EOF, timeout, or protocol failure puts the host into an error state; the user must explicitly acknowledge **Start New Backend Session** because the replacement process owns a new `EditorUVE` and v1 provides no Save command. Last-known `sceneDirty` is displayed as evidence only, never as a recoverability promise. A dirty connected session similarly requires an explicit discard confirmation before host shutdown.
