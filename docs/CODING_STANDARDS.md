...
Translate plane handles occupy the positive 20–60% square of each selected axis pair. Axis and endpoint candidates are evaluated before any plane candidate—this is an interaction contract, not merely draw order. A plane drag captures both world axes and solves pointer displacement through its 2×2 projected screen basis; snapping independently rounds both world coefficients before the resulting combined world delta passes through the existing parent-safe local-position conversion. Uniform/negative scale, free/trackball rotation, snapping refinements, and final selection-outline rendering remain deferred.
...
**Raw handles never cross Stop.** `IEntityManagerUVE` remains the canonical handle invalidation boundary: transient entity destruction publishes `EntityDestroyedEventUVE`, and restore publishes fresh `EntityCreatedEventUVE` handles. `EditorUVE` stores no pre-Play raw selected entity; it stores a root/child-index path relative to the serialized immutable snapshot and resolves it only after the snapshot's root order and hierarchy have been restored. Runtime spawn, despawn, and reparenting cannot shift that lookup because the transient hierarchy is discarded first. Future scripting, event-routing, or cache-owning systems must subscribe to entity lifecycle events or call `IsAliveUVE()` before each handle use.
