...
**Deliberately deferred:** offscreen texture compositing of the viewport, mesh-triangle or render-ID picking, non-collider selection, final visual selection outline/bounds rendering, viewport navigation controls, local/plane/rotate/scale gizmo handles, snapping, Play/Pause sandboxing, undo/redo, multi-selection, reflection-based/custom inspector drawers, native file dialogs, layout/preferences persistence, autosave policy changes, filesystem scanning, asset import/reimport, asset drag-and-drop, thumbnails, preview generation, and editor entity naming. These remain separate increments so every editor slice stays buildable, testable, and compatible with the existing renderer.
