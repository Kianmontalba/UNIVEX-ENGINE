...
**Right tabs and lower docks are composition state, not functionality claims.** Inspector, Import, and Signals are horizontal tabs within one fixed right panel. Only Inspector owns the existing entity Name/Transform mutation surface. Import and Signals communicate their unavailable backing integrations instead of faking filesystem import or script-signal behavior. FileSystem is the default lower dock and continues to expose only the copied, sorted `IAssetDatabaseUVE::GetRegisteredAssetsUVE()` record snapshot. Selecting Debugger, Animator, or AI Toolbar changes lower-workspace presentation without enabling unsupported service ownership, background jobs, scene mutation, or history entries. The `+ Add Dock` affordance is explicitly disabled until layout persistence and real dock registration are introduced.
