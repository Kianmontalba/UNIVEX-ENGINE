...
### Deterministic Motion Query Replay Workflow v1 (Increments 195–207)

**Replay fixtures are bounded value contracts, not runtime recordings.** `MotionQueryTraceReplayFixtureUVE` retains only versioned, sequence-ordered event facts under explicit event and payload caps. The codec rejects unsupported schemas, malformed structures, forbidden runtime identity/timestamp fields, unexpected fields, invalid compatibility identities, and oversized payloads. Replay comparison never uses wall-clock time, raw handles, logger ownership, or free-form unbounded diagnostics; field-level mismatch facts use a bounded mask and deterministic summary order.

**Named baseline state is native-owned and generation-guarded.** `MotionQueryTraceReplayBaselineRegistryUVE` stores at most the documented bounded number of copied fixtures in deterministic name order. `LoadMotionQueryReplayBaseline` must validate the bounded name and canonical payload, register and generation-aware-select the copy, publish it as the active comparison fixture, and increment the bridge revision only after acceptance. `ClearMotionQueryReplayBaseline` removes the named copy and clears the active selection when applicable. A stale `expectedRevision` rejects either command before mutation; a stale registry generation rejects named regression selection without comparing evidence.

**Regression comparison requires complete copied evidence.** `CompareMotionQueryLiveDebugSnapshotAgainstNamedBaselineUVE` rejects filtered or empty live-debug snapshots, selects only copied registry data, optionally enforces schema/sampler/normalization/source-generation compatibility, and returns explicit Match/Mismatch/NotFound/Stale/Filtered/Empty result facts. The stdio adapter emits these commands as bounded framed JSON; the managed parser accepts additive comparison fields with safe legacy defaults and renders facts only. C# must not load files, own fixtures, infer comparison semantics, or bypass the native bridge.
...
`ScriptRuntimeUVE` owns only copied script-instance values keyed by generational `Scene::EntityUVE` handles. It does not own entities, components, ECS pointers, scene lifetime, or engine services. Attach rejects invalid or duplicate handles, incompatible bytecode versions, oversized programs, and instance-cap overflow without mutation. Runtime state preserves legacy bounded int64 scalar slots, a separate bounded typed Vector3 store with finite-component validation, and a bounded per-entity VM context; compatible reloads preserve all state. The VM has an explicit caller-owned variant binding context and deterministic dependency-aware dispatch for the eight built-in Vector3 node IDs. Runtime state updates validate context identity/finite values, and mutating ticks execute enabled instances in canonical entity order while persisting copied typed outputs and accounting node failures. Entity-handle generation is part of identity and must never be reduced to an index-only key. Neither typed storage nor VM dispatch may introduce hidden engine-call bindings.
