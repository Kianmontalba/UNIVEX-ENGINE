...
| 3 | **Material and mesh asset workflow** | Inspector GUID binding, deterministic typed UVE envelope reimport, copied AssetManager failure diagnostics, copied Renderer3D invalid/pending/failed/fallback counts, deterministic raw model/texture/material source classification, and selected-entry Content Browser import/reimport capability facts are closed; remaining actual conversion backends, executable Content Browser actions, and previews must be audited before implementation. | Asset-backed mesh/material/texture path is deterministic, editor-visible, failure-safe, and covered by import, renderer, Inspector, bridge, and smoke tests. |
