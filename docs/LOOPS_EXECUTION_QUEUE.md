...
| 3 | **Material and mesh asset workflow** | Inspector GUID binding, deterministic typed UVE envelope reimport, copied AssetManager failure diagnostics, and copied Renderer3D invalid/pending/failed/fallback counts are closed; remaining raw model/texture/material source conversion, Content Browser actions, and previews must be audited before implementation. | Asset-backed mesh/material/texture path is deterministic, editor-visible, failure-safe, and covered by import, renderer, Inspector, and smoke tests. |
