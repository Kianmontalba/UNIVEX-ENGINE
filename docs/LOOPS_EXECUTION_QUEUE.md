...
| 5 | **Scene components and prefab maturity** | Runtime scene ownership and serialization are authoritative; camera, light, collider, rigid-body, audio-source, mesh, Script, Particle, and PrefabInstance validation plus bounded deterministic override persistence are now closed foundations for their authored data. | Caller-owned failure-atomic prefab Apply/Revert is proven with explicit baseline semantics, and validated ScriptComponent ownership now reconciles empty-path detach and graph-to-runtime attach without filesystem ownership; asset loading, replacement reload, particle runtime/renderer ownership, listener/mixer/effects depth, shape/filter breadth, constraints, character depth, editor commands, conflict handling, and source-prefab mutation require independent bounded increments. |
