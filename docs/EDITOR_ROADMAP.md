...
| **IMPLEMENTING** | **110** | **ScriptRuntime Per-Entity Bytecode Reload Safety v1**: validated program replacement, generation tracking, last-known-good retention on failure, and structured bytecode diagnostics. | Reload is transactional per live entity; invalid candidates never replace the active program, missing instances are reported explicitly, and the runtime remains the sole execution authority. Focused native runtime tests and full cross-compiler validation are required before merge. |
