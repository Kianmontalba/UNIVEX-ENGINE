...
| **IMPLEMENTING** | **113** | **Policy-Aware Native Plugin Registry Registration v1**: `NativePluginRegistryUVE` accepts an explicit capability policy overload and rejects disallowed manifests before registry mutation, while retaining the default allow-all compatibility path. | Registry mutation, duplicate IDs, capacity, scopes, generations, and lifecycle remain native registry authority. Focused policy-aware registry tests and full cross-compiler validation are required before merge. |
