...
| **IMPLEMENTING** | **100** | **Typed Data Table Asset-Manager Loader v1**: registers the existing envelope-backed loader for `DataTableUVE`, enables async ref-counted `AssetHandleUVE<DataTableUVE>` loads, and preserves generic manager failure semantics. | Loader registration is explicit and synchronous; individual loads remain owned by `AssetManagerUVE`. No hot reload, registry session mutation, editor bridge update, or managed pointer transfer is introduced. |
