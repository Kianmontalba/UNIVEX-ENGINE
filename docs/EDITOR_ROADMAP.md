...
| **IMPLEMENTING** | **98** | **Data Table `.uve*` Asset Store v1**: `AssetKindUVE::DataTable`, envelope-backed save/load helpers, serializer reuse, asset-kind validation, and failure-atomic destination replacement. | The store persists one validated table per file but does not scan directories, register asset database records, hot reload, infer schemas, mutate registry sessions, or add runtime bindings. |
