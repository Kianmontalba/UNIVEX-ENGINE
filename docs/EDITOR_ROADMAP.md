...
| **IMPLEMENTING** | **105** | **Visual Script Canvas Layout Persistence v1**: bounded deterministic JSON layout encoding/decoding, complete live-node validation, and native ApplyLayout undo support. | Graph, runtime, selection, filesystem, and managed ownership remain outside this increment; pan/zoom remains non-undoable session state. Focused native persistence tests and cross-compiler validation are required before merge. |
