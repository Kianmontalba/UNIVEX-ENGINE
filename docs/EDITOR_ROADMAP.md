...
| **IMPLEMENTING** | **92** | **Data Table Asset Envelope and Catalog v1**: versioned deterministic JSON envelope for validated native tables plus copied lexicographically sorted catalog descriptors. | Deserialization validates into a temporary table before replacement; the catalog owns descriptors only. Filesystem scanning, asset-database registration, content hashing, hot reload, editor bridge, managed UI, runtime binding, references, and visual-scripting nodes remain deferred. Native tests and full cross-compiler/CI validation are required before promotion to COMPLETED. |
