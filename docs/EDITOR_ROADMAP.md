...
| **IMPLEMENTING** | **97** | **Read-Only Data Table Preview Selection Protocol v1**: named revision-checked native request, stdio JSON transport, managed `BridgeCommand` serialization, and Avalonia catalog selection that requests a copied native preview. | Selection changes only bridge session presentation state. Unknown names, stale revisions, registry-free bridges, and malformed payloads are rejected without table mutation or managed/native pointer transfer. |
