...
| **IMPLEMENTING** | **104** | **EngineCore Data Table Pipeline Integration v1**: EngineCore registers the Data Table importer and typed loader during service initialization, making the complete pipeline available through `GetServicesUVE()`. | Headless EngineCore integration must prove CSV import and asynchronous typed load after `Init()`/`Load()`; service ownership and generic importer/manager boundaries remain unchanged. |
