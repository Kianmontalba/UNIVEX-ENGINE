...
| **IMPLEMENTING** | **94** | **Read-Only Data Table Detail Preview Bridge v1**: bounded typed column descriptors, native-formatted cell display values, additive stdio preview DTO, backward-compatible managed parsing, and presentation-only column/row preview. | C++ retains schema and row authority. No table mutation, filesystem access, asset-database registration, reference binding, hot reload, runtime binding, or visual-scripting node is introduced. Native/stdio/managed tests and full cross-compiler/CI validation are required before promotion to COMPLETED. |
