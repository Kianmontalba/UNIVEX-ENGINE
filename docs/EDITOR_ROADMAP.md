...
| **IMPLEMENTING** | **108** | **Managed Visual Script Breakpoint Presentation v1**: a bounded, non-interactive Scripting-panel list renders copied breakpoint node IDs from the debugger DTO. | The list is presentation-only and hit-test disabled; breakpoint creation, removal, and debugger execution remain native concerns. Focused managed build/test and full cross-compiler validation are required before merge. |
