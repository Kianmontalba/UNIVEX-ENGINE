...
| **IMPLEMENTING** | **90** | **Typed Data Table Core v1**: bounded native schemas and rows, Boolean/Integer/Number/String values, copied snapshots, deterministic diagnostics, and a failure-atomic CSV ingestion seam. | The caller supplies the schema; v1 does not infer types or own files, assets, ECS, renderer, managed runtime, editor bridge, hot reload, export, references, JSON/TSV/XLSX adapters, or visual-scripting bindings. Native tests and full cross-compiler/CI validation are required before promotion to COMPLETED. |
