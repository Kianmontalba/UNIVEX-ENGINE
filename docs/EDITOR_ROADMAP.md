...
| **IMPLEMENTING** | **112** | **Native Plugin Capability Policy v1**: an explicit bounded allowlist validates requested manifest capabilities before registry mutation, with deterministic rejection diagnostics for policy overflow, duplicate policy entries, invalid policy IDs, and disallowed capabilities. | The policy is static value data; it adds no dynamic loading, filesystem manifest authority, ABI negotiation, arbitrary callbacks, or ownership bypass. Focused policy tests and full cross-compiler validation are required before merge. |
