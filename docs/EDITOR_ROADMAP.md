...
| **IMPLEMENTING** | **103** | **Data Table Pipeline Bootstrap v1**: one explicit helper composes source importer registration for `.csv`/`.tsv`/`.json` with typed `DataTableUVE` asset-manager loader registration. | The helper owns no service or asset state and introduces no new cache, queue, registry, editor bridge, runtime, or managed authority. |
