...
| **IMPLEMENTING** | **109** | **Managed Visual Script Debugger Source Mapping Presentation v1**: the debugger source node ID is mapped to a copied canvas display label with a bounded fallback when the node is not in the visible snapshot. | Mapping uses only copied DTO values and cannot mutate the graph, debugger, or runtime. Focused managed build/test and full cross-compiler validation are required before merge. |
