...
| **IMPLEMENTING** | **99** | **Validated Data Table Asset-Database Registration v1**: explicit helper validates one `.uvetable` envelope before registering its lexical path and returning the existing or generated asset GUID. | Invalid extensions, missing files, wrong envelope kinds, and malformed table payloads reject before database mutation. No directory scan, import queue, hot reload, runtime binding, or managed pointer is introduced. |
