...
| **IMPLEMENTING** | **93** | **Read-Only Data Table Catalog Bridge v1**: bounded native catalog DTOs, additive stdio serialization, revision synchronization, backward-compatible managed parsing, and a presentation-only Avalonia Data Tables tab. | No catalog mutation request, filesystem access, asset-database ownership, editor-side table editing, runtime binding, references, hot reload, or visual-scripting node is introduced. Native/stdio/managed tests and full cross-compiler/CI validation are required before promotion to COMPLETED. |
