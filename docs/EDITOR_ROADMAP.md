...
> **Technology direction:** UniVex remains a native **C++20** engine/editor with CMake and engine-owned scene, history, renderer, and command authority. GLSL belongs to viewport, grid, selection, and gizmo visuals. JSON/TOML-style data belongs to versioned editor settings and presets. SVG/PNG/WebP belong to future icon/resource pipelines. Python is optional only for offline asset processing or build automation. The optional **C#/.NET host foundation is COMPLETED** as a separate headless-bridge client; docking, panel workflows, scene authoring UI, and native viewport hosting remain **PARTIAL** later milestones.
