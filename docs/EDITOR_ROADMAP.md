...
| **IMPLEMENTING** | **87** | **Managed Visual Scripting Canvas Presentation v1**: Avalonia canvas control renders copied nodes, links, typed-pin indicators, selection, pan/zoom, palette/diagnostic status, and routes named Undo/Redo, selection, move, and view commands through the existing C++ bridge. | C# remains presentation-only and stores only copied DTOs plus transient pointer state. No raw native references or OpenGL resources cross the bridge. Geometry/hit-testing/command emission tests, managed build/tests, Xvfb shell smoke, and full native/managed CI validation are required before promotion to COMPLETED. |
