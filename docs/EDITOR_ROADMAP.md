...
| **IMPLEMENTING** | **88** | **Developer Console and Diagnostics v1**: bounded native command registry, safe built-in help/clear/cvar commands, explicit CVAR facts, bounded output/history snapshots, named bridge requests, and managed lower-dock Console presentation. | C++ owns command execution and diagnostics; C# receives copied DTOs and submits bounded text only through registered named paths. Filesystem/process/network/ECS/renderer/plugin commands, remote console transport, and shipping-build policy remain deferred. Native/bridge/stdio/managed tests and full CI validation are required before promotion to COMPLETED. |
