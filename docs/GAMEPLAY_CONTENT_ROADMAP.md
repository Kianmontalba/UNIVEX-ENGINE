...
|---|---|---|---|
| **PARTIAL** | Core 3D nodes | `Node3D`, mesh/material references, camera variants, directional/point/spot lights, collision shapes, rigid/static/character bodies, areas, audio sources, and script attachments. | Each node/component needs runtime behavior, serialization, Inspector command path, validation, and tests; no empty node shells. |
