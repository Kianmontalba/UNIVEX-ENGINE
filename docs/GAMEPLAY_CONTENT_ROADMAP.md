...
|---|---|---|---|
| **PARTIAL** | Core 3D nodes | `Node3D`, mesh/material references, camera variants, directional/point/spot lights, collision shapes, rigid/static/character bodies, areas, audio sources, and script attachments. | Camera, light, collider, rigid-body, and audio-source validity plus invalid-scene rollback are proven; MeshComponent mesh/material reference coherence and rollback are also proven; camera/light/collider/rigid-body/audio authoring commands, shape breadth, constraints, character depth, mixer behavior, raw mesh import, and each remaining node/component still require runtime behavior, serialization, validation, and tests; no empty node shells. |
