...
| **One scene model** | Gameplay, editor, serializer, visual scripting, and physics operate on the same scene/entity ownership model. The Visual Scripting canvas now has copied native palette descriptors, managed search filtering, right-click node search, keyboard/click selection, cursor-position insertion, an 18-node production catalog covering Float math, Vector3 math, Boolean logic, and Has/Get Component entity queries, pure typed Vector3 math and copied Entity/Component query values, bounded typed VM values including Boolean storage, explicit dependency-aware VM dispatch for all 18 built-in node IDs, copied entity-component facts, per-entity typed runtime tick execution, a scene-owned ScriptEntityQueryAdapterUVE that maps caller-registered native component types to binding-ordered copied presence facts for alive entities, and a query-aware ScriptRuntimeUVE tick path that refreshes those facts immediately before each enabled instance executes; Increment 274 added bounded copied ScriptVmTraceEventUVE records for node execution, typed value transfer, query-fact refresh, completion, and failure, with deterministic refresh-first ordering and truncation evidence; Increment 275 now copies debugger trace history through ScriptSourceMappingUVE, EditorBridgeUVE, and the managed Avalonia snapshot/status path alongside source-mapped breakpoints; broader runtime/editor coverage, flow control, debugger controls beyond read-only trace presentation, and engine-call bindings remain partial. |
