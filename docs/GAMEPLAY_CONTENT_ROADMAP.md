...
|---|---|---|---|
| **PARTIAL** | Core 3D nodes | `Node3D`, mesh/material references, camera variants, directional/point/spot lights, collision shapes, rigid/static/character bodies, areas, audio sources, and script attachments. | Camera and light component validity plus invalid-scene rollback are proven; camera/light authoring commands, advanced lighting, and each remaining node/component still require runtime behavior, serialization, validation, and tests; no empty node shells. |
