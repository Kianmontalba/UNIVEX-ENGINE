...
| **PARTIAL** | Asset-type importers | Extend the typed-envelope foundation with deliberately selected model, texture, audio, material, animation, and shader source-import contracts. | `.uvemodel`/`.uvetex`/`.uvemat`/`.uveshader` copy-and-register and deterministic raw model/texture/material source classification are complete; raw-source parser licensing, source/derived-data transforms, deterministic metadata, and representative fixtures are still required. |
