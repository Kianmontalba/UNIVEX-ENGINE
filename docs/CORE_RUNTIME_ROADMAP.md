...
|---|---|---|---|
| **PARTIAL** | Scene components and user-facing nodes | Introduce only supported components/nodes such as richer camera, mesh, light, collider, rigid-body, audio-source, animation, particle, and script attachments. | Camera and light value validation plus invalid-scene rollback are complete; each remaining component still requires serialization, runtime ownership, editor command path, validation, and tests; no placeholder node catalog. |
