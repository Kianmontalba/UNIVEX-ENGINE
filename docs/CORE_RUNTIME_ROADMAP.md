...
| **PARTIAL** | Asset-type importers | Extend the typed-envelope foundation with deliberately selected model, texture, audio, material, animation, and shader source-import contracts. | `.txt` bounded parsing and `.uvemodel`/`.uvetex`/`.uvemat`/`.uveshader` copy-and-register are complete; raw model/texture/material/audio/animation source-parser licensing, source/derived-data transforms, deterministic metadata, and representative fixtures are still required. |
