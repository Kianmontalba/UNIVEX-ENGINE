...
| **COMPLETED** | **Scene Component Slice — Collider Validation v1** | `ColliderComponentUVE` exposes a shared physics-safe validity predicate for finite positive half-extents, nonzero collision layer, bounded friction/restitution, and positive finite density; scene restore rejects invalid payloads and the collider world-AABB cache asserts the runtime precondition. | Validation does not add shape types, change collision filtering, wire density to mass, or repair malformed data. Collision masks, shape breadth, rigid-body depth, and editor repair UX remain separate partial work. |
