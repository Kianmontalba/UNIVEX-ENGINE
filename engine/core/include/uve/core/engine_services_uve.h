...
/// (Logger, Timer, EventSystem, MemoryManager), built once EngineCoreUVE has
/// constructed all four. Any future subsystem that needs Logger/Timer/
/// EventSystem/MemoryManager access should receive an EngineServicesUVE&
...
/// the one intentional exception to that rule (see docs/CODING_STANDARDS.md).
/// Referenced through the ILoggerUVE/ITimerUVE/IEventSystemUVE/
/// IMemoryManagerUVE interfaces, not the concrete types, so a future
/// substitute implementation of any of the four requires no change here.
...
public:
    EngineServicesUVE(Debug::ILoggerUVE& logger, Utilities::ITimerUVE& timer,
                       ...
                       Memory::IMemoryManagerUVE& memoryManager) noexcept;
