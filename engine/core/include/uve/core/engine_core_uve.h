...
/// EngineCoreUVE owns the foundational engine services (Logger, Timer,
/// EventSystem) and drives the canonical engine lifecycle: Init -> Load ->
...
public:
    ...
    /// Constructs and initializes Logger, Timer, and EventSystem in that
    /// order (Logger first — every later step and every other system may
    /// need to log or UVE_ASSERT during its own setup), then builds
    /// EngineServicesUVE from all three. Transitions Uninitialized ->
    ...
    /// EventSystem, then Timer, then Logger — the exact reverse of Init()'s
    /// construction order — logging the final message before the logger
    /// itself is torn down last.
    void Shutdown();
    ...
    /// references. Valid only between Init() and Shutdown().
