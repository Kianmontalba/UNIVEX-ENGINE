...
# Start the managed fixed-region shell under a virtual display (Ctrl+C stops it).
# It renders copied C++ Scene/Inspector/FileSystem snapshots and never autosaves layout on startup or close.
xvfb-run -a dotnet run --project editor/managed/UniVex.EditorHost/UniVex.EditorHost.csproj --no-build --no-restore
...
**C# Editor Host Foundation v1** adds an optional .NET 8/Avalonia connection host and a local length-prefixed JSON-RPC bridge transport. The host launches one separate `uve_editor --bridge-stdio` child, completes a versioned `bridge.hello` handshake, shows copied backend capability/snapshot facts, and presents compatible, incompatible, failed, and disconnected states. Bridge stdio is intentionally **headless-only and mutually exclusive with native ImGui in that process**: it creates no GLFW window, ImGui overlay, or OpenGL resource. Protocol stdout contains framed JSON-RPC only; logs remain on stderr/file sinks. A crash, EOF, timeout, or compatibility failure never auto-restarts the backend. **Start New Backend Session** requires acknowledgement that it creates a fresh `EditorUVE` and may have lost unsaved in-memory work because bridge v1 has no Save command; a dirty connected session similarly requires an explicit discard confirmation before host shutdown.

**C# Editor Shell & Docking v1** now builds a fixed-region, resizable managed presentation around that existing connection host: menu/workspace strips plus left Scene, center Viewport, right Inspector/Import/Signals, and lower FileSystem/Debugger/Animator/AI Toolbar tab regions. It deliberately uses no floating or tear-off dock runtime. Its application-local layout file is presentation-only: recognized legacy v0 records migrate in memory to v1, known v1 records validate and normalize, and corrupt or unknown-future records fall back safely without an automatic overwrite. The layout is written only through **Save Shell Layout**; resizing, tab/workspace selection, reconnect, crash, and clean close never autosave.

**C# Hierarchy, Inspector & Content Browser v1** replaces the managed Scene, Inspector, and FileSystem placeholders with real presentation over strictly parsed, immutable, bounded C++ bridge snapshots. The Scene panel renders native hierarchy depth/type/selection facts and issues only named revision-protected selection or hierarchy-filter commands. The Inspector renders native no/single/multi-selection context, ancestry, parent, and drawer identifiers; its sole shipped edit is the existing C++-validated selected-name command. The FileSystem panel renders the native cached project-index view, C++-computed breadcrumbs/type/asset-registration facts, and sends only named native-validated cached navigation/filter/focus/refresh/entry-selection intent. C++ retains ECS, scene writes, command/history/Play guards, project-index refresh, filesystem validation, backend lifecycle, renderer, and OpenGL ownership. Generic component reflection, Transform/reparent/create/delete authoring, import mutation, native viewport hosting, and OpenGL ownership remain later increments.

**GLSL Viewport Visuals v1** adds an additive renderer-owned `EditorViewportVisuals` pass between the HDR scene pass and tone mapping. The pass composes a bounded analytic grid with major-axis accents, active-selection outline, camera-facing X/Y/Z axis widget strokes, and gizmo-axis state markers. C++ owns the copied normalized viewport/selection/camera facts, pass ordering, explicit source-alpha blend state, and ImGui input semantics; GLSL owns appearance only. The pass is disabled unless native editor visual state is published, so runtime/game rendering remains unchanged. Shader parity, GCC, Clang, CI/Ninja, 751 native CTest cases, real OpenGL 4.5/Xvfb editor smoke, managed 35-test host suite, bridge probe, and managed shell no-autosave smoke passed. The managed shell still does not host native viewport pixels or OpenGL resources.
